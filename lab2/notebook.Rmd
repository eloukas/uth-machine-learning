...
GOOD data is the basis of any sort of regression model, because we use
...
------------------------------------------------------------------------
### 8          _LINEAR REGRESSION USING R

###                                         _2.3 THE EXAMPLE DATA_
------------------------------------------------------------------------
...
function called ```{r}extract_data()``` , which was specifically written for reading
the CPU DB file.
...
```{r}int92.dat , fp92.dat , int95.dat , fp95.dat , int00.dat , fp00.dat , int06.dat , ```
and ```{r}fp06.dat``` .
The data frame ```{r}int92.dat``` contains the data from the CPU DB database
...
(Fp1992) benchmarks, and so on. I use the ```{r}.dat``` suffix to show that the
...
```{r}int92.dat``` , truncated to fit within the page:
nperf
1
9.662070
2
...
A N I NTRODUCTION TO D ATA M ODELING
11CHAPTER 2. UNDERSTAND YOUR DATA
